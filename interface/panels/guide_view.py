...
class GuidePanel(QWidget):
    ...
    def __init__(self, controller):
        ...
        self.assets_dir = os.path.join(os.getcwd(), "interface/panels/assets").replace("\\", "/")
    ...
    def setup_ui(self):
        ...
        self.tabs_main_widget.setFixedHeight(36)
        ...
        self.tabs_header_layout.setContentsMargins(5, 5, 5, 0)
        self.tabs_header_layout.setSpacing(5)

        # -- Bouton Burger --
        self.btn_burger = QPushButton()
        self.btn_burger.setIcon(QIcon(os.path.join(self.assets_dir, "menu.svg")))
        self.btn_burger.setIconSize(QSize(14, 14))
        self.btn_burger.setFixedSize(24, 24)
        ...
        # [RESSORT 1] Pousse les éléments suivants vers le centre
        nav_layout.addStretch()

        # --- CENTRE : Navigation étapes ---
        self.btn_prev = self._create_btn("chevron-left.svg", self.controller.nav_previous)
        ...
        self.btn_next = self._create_btn("chevron-right.svg", self.controller.nav_next)

        # Ajout du bloc central au layout
        nav_layout.addWidget(self.btn_prev)
        nav_layout.addWidget(self.entry_step)
        nav_layout.addWidget(self.lbl_total)
        nav_layout.addWidget(self.btn_next)

        # [RESSORT 2] Pousse les éléments suivants vers la droite
        nav_layout.addStretch()

        # --- DROITE : Outils (Auto & Clavier) ---
        self.btn_auto = self._create_btn("plane.svg", self._toggle_auto)
        self.btn_auto.setCheckable(True)
        self.btn_auto.setChecked(True)
        self.update_auto_btn_color()

        self.btn_keyboard = self._create_btn("keyboard.svg", self._toggle_keyboard)
        ...
        nav_layout.addSpacing(5)  # Petit espace entre les deux boutons
        nav_layout.addWidget(self.btn_keyboard)
    ...
    def _create_btn(self, icon_name, cmd):
        """ Crée un bouton avec une icône SVG depuis le dossier assets """
        btn = QPushButton()
        ...
        # Style de base (sera écrasé pour les boutons toggle)
        btn.setStyleSheet("""
            QPushButton { 
                background: #333344; 
                border-radius: 6px; 
    ...
    def update_auto_btn_color(self):
        if self.btn_auto.isChecked():
            ...
            self.btn_auto.setStyleSheet("""
                ...
                QPushButton:hover { background-color: #2a4e38; }
            """)
        else:
            # INACTIF : Sombre et grisé
            self.btn_auto.setStyleSheet("""
                QPushButton { 
                ...
                QPushButton:hover { background-color: #2d2d35; }
            """)
    ...
    def update_keyboard_btn_color(self):
        if self.btn_keyboard.isChecked():
            # ACTIF
